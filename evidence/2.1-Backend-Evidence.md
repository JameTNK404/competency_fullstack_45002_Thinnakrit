# หลักฐานการประเมิน 2.1 Backend API (20 คะแนน)

*(ลำดับตาม readme-plan.md Rubric | Viewport: curl | สร้างอัตโนมัติจาก clean_and_collect.sh)*

---

## ตารางสรุปผลภาพรวม

| # | รายการประเมิน (Rubric) | TC | HTTP Status | ผล |
|:---:|---|:---:|:---:|:---:|
| 1 | สเปก OpenAPI/Swagger พร้อมใช้งาน | TC-BE-01 | 200 | ✅ PASS |
| 2 | Search (q) found / not found | TC-BE-02a/b | 200 | ✅ PASS |
| 3 | Sort asc/desc ลำดับต่างกัน | TC-BE-03 | 200 | ✅ PASS |
| 4 | Pagination + meta.total/page/pageSize | TC-BE-04 | 200 | ✅ PASS |
| 5 | Idempotent q+sort+page (diff=0) | TC-BE-05 | 200 | ✅ PASS |
| 6 | Login → 200+token / No token → 401 | TC-BE-06a/b | 200 / **401** | ✅ PASS |
| 7 | Admin เห็นผลทุก evaluatee | TC-BE-07 | 200 | ✅ PASS |
| 8 | Evaluator: own→200 / other→403 (IDOR) | TC-BE-08a/b | 200 / **403** | ✅ PASS |
| 9 | Evaluatee: self→200 / other→403 (IDOR) | TC-BE-09a/b | 200 / **403** | ✅ PASS |
| 10 | Score Range Validation (0→400, 5→400, 3→200) | TC-BE-10a/b/c | **400/400**/200 | ✅ PASS |
| 11 | Evidence Required (yes_no=1+no file → 400) | TC-BE-11 | **400** | ✅ PASS |
| 12 | 413 Payload Too Large (>10MB) | TC-BE-12 | **413** | ✅ PASS |
| 13 | 415 Unsupported Media Type (.exe) | TC-BE-13 | **415** | ✅ PASS |
| 14 | Create Assignment (ไม่ซ้ำ) → 201 | TC-BE-14 | **201** | ✅ PASS |
| 15 | Duplicate Assignment → 409 | TC-BE-15 | **409** | ✅ PASS |
| 16 | Reports: Normalized /60 | TC-BE-16 | 200 | ✅ PASS |
| 17 | Reports: Progress per Department | TC-BE-17 | 200 | ✅ PASS |
| 18 | Filter idempotency (diff=0) | TC-BE-18 | 200 | ✅ PASS |
| 19 | E2E Flow (Login→assign→submit→report) | TC-BE-19 | 200 | ✅ PASS |
| 20 | Security refs (IDOR 403 + 413 + 415) | TC-BE-20 | 403/413/415 | ✅ PASS |

---

## รายละเอียดหลักฐานแต่ละ TC
...
```json
{
  ...
  "items": [
    ...
    },
    {
      ...
      "topic_id": 2,
      "code": "T2-CHART",
      "name_th": "แผนภูมิ/ตาราง",
      "description": "แผนผังที่นั่ง กฎห้องเรียน ตารางเวร",
      "type": "yes_no",
      "weight": "1.00",
      "min_score": 1,
      "max_score": 4,
    ...
    },
    {
      ...
      "topic_id": 3,
      "code": "T3-IDP",
      "name_th": "แผนพัฒนาตนเอง",
      "description": "เป้าหมาย/แนวทางพัฒนาตามสายงาน",
      "type": "yes_no",
      "weight": "1.00",
      "min_score": 1,
      "max_score": 4,
      ...
      "topic_name": "การพัฒนาตนเองและพัฒนาวิชาชีพ"
    }
  ],
  "meta": {
    "total": 3,
...
```json
{
  ...
  "data": {
    ...
    "updated_at": "2026-02-25T12:04:44.000Z",
    "submitted_at": null
  }
}
```

### TC-BE-11_no_attach_400
...
```json
{
  ...
  "items": [
    ...
    },
    {
      ...
      "department": 2,
      "period_name": "การประเมินครูประจำปี 2568",
      "evaluator_name": "กรรมการประเมินเครื่องกล",
